      ...
      g_inv_m(fem)
{
    ...
    for (int k = 0; k < n_shared; ++k)
    {
        ...
        _b[subspace1].push_back({face_index1, n_shared + k}); // lambda0 of subspace1 == lambda1 of subspace0

        _bt[subspace0].push_back({face_index0, n_shared + k}); // lambda1
...
void DDH::action(const double * x, double * y) const
{
    ...
    if (n_basis == 4)
        ...
        ddh_action<8, DDH_BLOCK_SIZE/8>(efem.get(), g_ndof, g_elem, n_domains, n_basis, n_lambda, nt, omega, dt, g_inv_m, s_lambda, B, dualB, D, g, m, inv_m, H, wh_filter, cs, sn, x, y, d_lambda, d_update);
    else
