...
parser = argparse.ArgumentParser(description="Visualize Helmholtz solution")
parser.add_argument("xy", help="Path to xy binary file")
parser.add_argument("uv", help="Path to uv binary file")
parser.add_argument(
    "--grid",
    type=int,
    default=200,
...
x, y = np.fromfile(args.xy, dtype=np.float64).reshape((2, -1), order="F")
u, v = np.fromfile(args.uv, dtype=np.float64).reshape((-1, 2), order="F").T
