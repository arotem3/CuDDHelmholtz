...
constexpr double maxvel = 5.0; // maximum reciprocal of alpha (= 1/min(alpha))
...
int main()
{
    ...
    // compute b = (f, phi)
    MassMatrix3D M(fem);
    l2_project(M, [=] __device__(double3 x) -> double { return f(x); }, d_b);

    to_file("solution/b.0000", N, b.host_read());

    // build variable coefficient GridFunc3D
    auto gf_alpha = gridfunc(fem, [=] __device__(double3 x) -> double { return alpha(x); });
    ...
    WaveEquation3D W(fem, fs, gf_alpha);
